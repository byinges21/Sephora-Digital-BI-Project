WITH base AS (
  SELECT channel, SUM(sessions) AS sessions, SUM(product_views) AS product_views, SUM(add_to_cart) AS add_to_cart,
         SUM(checkout_starts) AS checkout_starts, SUM(orders) AS orders, SUM(revenue) AS revenue
  FROM fact_sessions
  GROUP BY channel
)
SELECT channel, sessions, product_views, add_to_cart, checkout_starts, orders,

WITH channel_funnel AS (
  SELECT channel, SUM(product_views) AS views, SUM(add_to_cart) AS carts,
         SUM(checkout_starts) AS checkouts, SUM(orders) AS orders
  FROM fact_sessions
  GROUP BY channel
)
SELECT channel, views, carts, checkouts, orders,
...
SELECT channel, traffic_source, SUM(sessions) AS sessions, SUM(orders) AS orders,
